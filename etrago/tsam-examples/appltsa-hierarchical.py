...
if 'READTHEDOCS' not in os.environ:
    ...
    from etrago.cluster.snapshot import snapshot_clustering, daily_bounds, apply_funcs, couple_design_periods, decouple_design_periods, inter_intra_soc
...
for count in mylist: 
    ...
    network=network_aux
    ###rename folder:
    clusterMethod = 'hierarchical' #'hierarchical', 'k_means', 'k_medoids', 'hierarchicalwithpeaks'
    ...
    network, snapshot_map, day_map = snapshot_clustering(
            network, how='daily', clusters=args['snapshot_clustering'],clusterMethod = clusterMethod, normed=True)
    base = np.ones(24, dtype=int)
    periods = base*0
    newcount=len(np.unique(day_map))
    for i in range(1,newcount):
        periods = np.concatenate([periods,base*i])
    extra_functionality = couple_design_periods(network_aux.snapshots,snapshot_map)  # daily_bounds ,couple_design_periods, decouple_design_periods, inter_intra_soc
    ...
    try:
        network.lopf(
                ...
                solver_name='gurobi',
                solver_options =  {
                    'Method': 2,  #-1: authomatic, 2: Barrier
                    'threads': 4, 
                    'LogFile': directory + '/gurobi.log',  
                    'OutputFlag': 1,
                    'BarConvTol' : 1.e-4, #1.e-12 ###1e-8 # [0..1]
                    'Crossover' : 0,   # 0 deactivate or -1: active (default)
                    'FeasibilityTol' : 1e-4, ###1e-6 # [1e-9..1e-2]
                    'BarHomogeneous' : 1, #to deal with infeasibility
                    },
